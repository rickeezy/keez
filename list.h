 ...
 * you want! */ //instead of using int use elem
typedef int elem;
 ...
 * next pointer. */
struct node {
	elem value; //value is an int but called elem
	struct node *next;
};
typedef struct node node_t; //type define for node called node_t
...
typedef struct list list_t;  //list_t is alias for sturcture list
